...
const { isCAPOperatorChart, isFlexibleTemplateChart, injectTemplateFunction } = require('./util')

const Mustache = require('mustache')

module.exports = class CapOperatorAddFlexibleTemplatesPlugin extends cds.add.Plugin {
    ...
    async run() {
        if (cds.cli.options.force) await rimraf('chart')
            ...
            if (!isCAPOpChart) {
                throw new Error(`Existing 'chart' folder is not a CAP Operator helm chart. Run 'cds add cap-operator-with-flexible-templates --force' to overwrite.`)
        ...
        await copy(join(__dirname, '../files/chartFlexibleTemplates/templates')).to('chart/templates')
        await cds.add.merge(__dirname, '../files/chartFlexibleTemplates/Chart.yaml.hbs').into('chart/Chart.yaml', { project })
        await copy(join(__dirname, '../files/chartFlexibleTemplates/values.yaml')).to('chart/values.yaml')
        await copy(join(__dirname, '../files/chartFlexibleTemplates/values.schema.json')).to('chart/values.schema.json')
    ...
    async combine() {
        ...
        if (!isFlexibleTemplateChart('chart')) return
