...
module.exports = class CapOperatorAddPlugin extends cds.add.Plugin {
    ...
    async run() {
        ...
        if (exists('chart')) {
            await this.handleExistingChart()
            return
        }
        ...
        if (isConfigurable) {
            await this.createConfigurableChart(project, isServiceOnly)
        } else {
            await this.createSimpleChart(project, withTemplates, isServiceOnly)
        }
    ...
    async combine() {
        ...
        if (!isConfigurableTemplateChart('chart')) {
            if ((hasApprouter || exists('approuter')) && !isServiceOnly)
                await addIf(true, '../files/approuter.yaml.hbs')
    ...
    async copySimpleChartTemplates(isServiceOnly) {
        ...
        const src = isServiceOnly
            ? '../files/chart/templates/cap-operator-cros-svc.yaml'
            : '../files/chart/templates/cap-operator-cros.yaml'

        await copy(join(__dirname, src)).to('chart/templates/cap-operator-cros.yaml')
    }
    ...
    async handleExistingChart() {
        const isConfigurable = isConfigurableTemplateChart('chart')
        const serviceOnly = isServiceOnlyChart('chart')
        ...
        if (!exists('chart/templates') && cds.cli.options['with-templates']) {
            await this.copySimpleChartTemplates(serviceOnly)
    ...
    async createConfigurableChart(project, isServiceOnly) {
        await copy(join(__dirname, '../files/configurableTemplatesChart/templates/_helpers.tpl')).to('chart/templates/_helpers.tpl')
        await copy(join(__dirname, '../files/commonTemplates/')).to('chart/templates/')
        await write(getConfigurableCapOpCroYaml(project, isServiceOnly)).to('chart/templates/cap-operator-cros.yaml')
    ...
    async createSimpleChart(project, withTemplates, isServiceOnly) {
        ...
        if (withTemplates)
            await this.copySimpleChartTemplates(isServiceOnly)
    }
