...
module.exports = class CapOperatorAddPlugin extends (cds.add?.Plugin ?? require('@sap/cds-dk/lib/init/plugin')) {
    ...
    async adoptIncompatibleChanges(isConfigurable) {
        ...
        const updatedValuesYaml = yaml.parse(await read(valuesPath))
        if (updatedValuesYaml.btp?.provider !== undefined) {
            console.log("⚠️  'btp.provider' is deprecated — removing from chart files")

            delete updatedValuesYaml.btp.provider
            await write(yaml.stringify(updatedValuesYaml)).to(valuesPath)
            console.log("⚠️  Updated values.yaml")
