...
module.exports = class CapOperatorAddPlugin extends cds.add.Plugin {
    ...
    async handleMtaIntegration(project) {
        ...
        // Update consumedBTPServices for router workload to includes all service bindings as router takes care of getDependencies
        const workloads = updateValuesMap.get('workloads')
        const serviceBindingNames = [...updateValuesMap.get('serviceBindings').values()].map(binding => binding.name)
