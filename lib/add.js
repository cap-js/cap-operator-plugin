...
module.exports = class CapOperatorAddPlugin extends cds.add.Plugin {
    ...
    options() {
        return {
            ...
            'generate-runtime-values': {
                type: 'boolean',
                help: 'Generate runtime values file for the chart.'
            },
        }
    }
    ...
    async run() {
        if (cds.cli.options.force) await rimraf('chart')
            ...
            if (cds.cli.options['generate-runtime-values'])
    ...
    async generateRuntimeValues() {
        ...
        // const shootDomain = await input({ message: 'Enter your cluster domain: ' })
        // const globalAccountId = await input({ message: 'Enter your global account ID: ' })
        // const tenantId = await input({ message: 'Enter your tenant ID: ' })
        // const hanaDatabaseId = await input({ message: 'Enter your HANA database ID: ' })
