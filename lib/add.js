...
const { isCAPOperatorChart, isConfigurableTemplateChart, injectTemplateFunction, transformValuesAndFillCapOpCroYaml, removeSectionFromYaml } = require('./util')
...
module.exports = class CapOperatorAddPlugin extends cds.add.Plugin {
    ...
    async run() {
        ...
        let project = {
            ...await cds.add.readProject(),
            isService: () => {
                return cds.cli.options['with-service-only'] ? true : false
            },
            isApp: () => {
                return cds.cli.options['with-service-only'] ? false : true
            }
        }
        ...
        if (cds.cli.options['with-configurable-templates']) {
            ...
            if (cds.cli.options['with-templates']){
                await copy(join(__dirname, '../files/chart/templates')).to('chart/templates')
                removeSectionFromYaml(join(cds.root, 'chart/templates/cap-operator-cros.yaml'), 'provider:')
            }
        }
