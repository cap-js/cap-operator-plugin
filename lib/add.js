...
module.exports = class CapOperatorAddPlugin extends cds.add.Plugin {
    ...
    async run() {
        if (cds.cli.options.force) await rimraf('chart')
            ...
            if (!isCAPOpChart) {
                throw new Error(`Existing 'chart' folder is not a CAP Operator helm chart. Run 'cds add cap-operator --force' or 'cap-operator-with-templates --force' to overwrite.`)
        ...
        if (process.env.WITH_MTA) {
            const { hasMta } = project
            if (!hasMta) throw new Error(`mta is not added to this project. Run 'cds add mta'.`)

            const mtaTransformer = new MtaTransformer(process.env.WITH_MTA, process.env.WITH_MTA_EXTENSIONS ? process.env.WITH_MTA_EXTENSIONS.split(',') : [])
        ...
        console.log("Once values.yaml is updated, run 'cds build' to generate the helm chart. You can find the generated chart in the 'gen' folder.")
    }
