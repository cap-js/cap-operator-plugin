...
module.exports = class CapOperatorAddPlugin extends cds.add.Plugin {
    ...
    async adoptIncompatibleChanges(isConfigurable) {
        const valuesPath = join(cds.root, 'chart/values.yaml')
        let valuesYaml = yaml.parse(await read(valuesPath))
        let changed = false

        if (valuesYaml.btp?.globalAccountId !== undefined) {
            ...
            changed = true
            valuesYaml = yaml.parse(await read(valuesPath))
        }

        if (valuesYaml.btp?.provider !== undefined) {
            console.log("⚠️  'btp.provider' is deprecated since CAP Operator v0.31.0 — removing from chart files")

            delete valuesYaml.btp.provider
            await write(yaml.stringify(valuesYaml)).to(valuesPath)
            console.log("⚠️  Updated values.yaml")
            ...
            if (exists(croPath)) {
                const croContent = await read(croPath)
                const updated = croContent.replace(
                    /\s*provider:\n\s*subDomain: \{\{ \.Values\.btp\.provider\.subdomain \}\}\n\s*tenantId: \{\{ \.Values\.btp\.provider\.tenantId \}\}\n/,
                    '\n'
                )
                if (updated !== croContent) {
                    await write(updated).to(croPath)
                    console.log("⚠️  Updated chart/templates/cap-operator-cros.yaml")
                }
            }
    ...
    async createConfigurableChart(project) {
        ...
        await write(getHelperTpl(project, true)).to('chart/templates/_helpers.tpl')
    ...
    async copySimpleChartTemplates(project) {
        await copy(join(__dirname, '../files/commonTemplates/')).to('chart/templates/')
        await write(getHelperTpl(project, false)).to('chart/templates/_helpers.tpl')
        await write(getDomainCroYaml(project)).to('chart/templates/domain.yaml')
    ...
    async handleMtaIntegration(project) {
        ...
        // Update consumedBTPServices for router workload to include all service bindings except html5-apps-repo app-host plan as router takes care of getDependencies
        const workloads = updateValuesMap.get('workloads')
        const serviceInstances = updateValuesMap.get('serviceInstances')
        const serviceBindings = updateValuesMap.get('serviceBindings')

        // Filter out html5-apps-repo app-host plan bindings
        const serviceBindingNames = [...serviceBindings.values()]
            .filter(binding => {
        ...
        for (const [key, workload] of workloads) {
            if (workload.deploymentDefinition?.type === 'Router') {
                workload.consumedBTPServices = serviceBindingNames
