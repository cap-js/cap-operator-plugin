...
module.exports = class CapOperatorAddPlugin extends cds.add.Plugin {
    ...
    async run() {
        ...
        if (exists('chart')) {
            ...
            if (!exists('chart/templates') && cds.cli.options['with-templates']) {
                await this.copyTemplates(isServiceOnlyChart('chart'))
        ...
        const project = this.readProject(cds.cli.options['with-service-only'] ? true : false)
        ...
        if (cds.cli.options['with-configurable-templates']) {
            ...
            if (cds.cli.options['with-templates'])
                await this.copyTemplates(cds.cli.options['with-service-only'])
        }
    ...
    async copyTemplates(isServiceOnly) {
    ...
    readProject(isServiceOnly) {
        return new Proxy(cds.add.readProject(), {
            get(target, prop) {
                if (prop === 'isService') {
                    return isServiceOnly == true
                }
                if (prop === 'isApp') {
                    return isServiceOnly == false
                }
                return target[prop]
            },
            has(target, prop) {
