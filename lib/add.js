...
module.exports = class CapOperatorAddPlugin extends cds.add.Plugin {
    ...
    async generateRuntimeValues() {
        ...
        if(cds.cli.options['generate-runtime-values-via-input-yaml']) {
            ...
            if (!answerStruct['appName'] || !answerStruct['capOperatorSubdomain'] || !answerStruct['clusterDomain'] || !answerStruct['globalAccountId'] || !answerStruct['providerSubdomain'] || !answerStruct['tenantId'] || !answerStruct['hanaInstanceId'] || !answerStruct['imagePullSecret'])
                throw new Error(`'appName', 'capOperatorSubdomain', 'clusterDomain', 'globalAccountId', 'providerSubdomain', 'tenantId', 'hanaInstanceId' and 'imagePullSecret' are mandatory fields in the input yaml file.`)

        } else {
            const answer = await ask(['Enter app name for deployment: ', project['appName']],['Enter CAP Operator subdomain (In kyma cluster it is "cap-op" by default): ', 'cap-op'],['Enter your cluster shoot domain: '], ['Enter your global account ID: '], ['Enter your provider subdomain: '], ['Enter your provider tenant ID: '], ['Enter your HANA database instance ID: '], ['Enter your image pull secrets: '])
        ...
        for (const [workloadKey, workloadDetails] of Object.entries(valuesYaml.workloads)) {
            ...
            const cdsConfigValue = Mustache.render('{"requires":{"cds.xt.DeploymentService":{"hdi":{"create":{"database_id":"{{hanaInstanceId}}"}}}}}', answerStruct)
            ...
            if (workloadDetails.deploymentDefinition && workloadDetails.deploymentDefinition.type === 'CAP') {
                ...
                if (index > -1)
                    runtimeValuesYaml['workloads'][workloadKey]['deploymentDefinition']['env'][index] = {name: 'CDS_CONFIG', value: cdsConfigValue}
                else
                    runtimeValuesYaml['workloads'][workloadKey]['deploymentDefinition']['env'].push({name: 'CDS_CONFIG', value: cdsConfigValue})
            }
            ...
            if (workloadDetails.jobDefinition && workloadDetails.jobDefinition.type === 'TenantOperation') {
                ...
                if (index > -1)
                    runtimeValuesYaml['workloads'][workloadKey]['jobDefinition']['env'][index] = {name: 'CDS_CONFIG', value: cdsConfigValue}
                else
                    runtimeValuesYaml['workloads'][workloadKey]['jobDefinition']['env'].push({name: 'CDS_CONFIG', value: cdsConfigValue })
            }
