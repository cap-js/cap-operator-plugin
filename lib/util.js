...
function getConfigurableCapOpCroYaml(project, isServiceOnly) {
  const lines = []
...
spec:
  ...
  if (isServiceOnly == undefined || isServiceOnly === false) {
    push(`  provider:
...
spec:
  ...
  const { hasDestination, hasHtml5Repo, hasXsuaa, hasApprouter, hasMultitenancy, hasIas } = project
  ...
  if (hasXsuaa) push('    - {{ include "originalAppName" $ }}-uaa-bind')
  if (hasIas) push('    - {{ include "originalAppName" $ }}-identity-bind')
  if (hasMultitenancy && hasXsuaa) {
    push('    - {{ include "originalAppName" $ }}-saas-registry-bind')
    push('    - {{ include "originalAppName" $ }}-service-manager-bind')
  }
  if (hasMultitenancy && hasIas) {
    push('    - {{ include "originalAppName" $ }}-subscription-manager-bind')
    push('    - {{ include "originalAppName" $ }}-service-manager-bind')
  }

  push(`    deploymentDefinition:`)
  if (isServiceOnly) {
    push(`      type: Service
      ports:
      - name: server-port
    ...
    push(`      image: {{ .Values.workloads.server.image }}
  ...
  if (hasApprouter) {
    ...
    if (hasXsuaa) push('    - {{ include "originalAppName" $ }}-uaa-bind')
    if (hasIas) push('    - {{ include "originalAppName" $ }}-identity-bind')
    if (hasMultitenancy && hasXsuaa) push('    - {{ include "originalAppName" $ }}-saas-registry-bind')
    if (hasMultitenancy && hasIas) push('    - {{ include "originalAppName" $ }}-subscription-manager-bind')
    if (hasDestination) push('    - {{ include "originalAppName" $ }}-destination-bind')
    if (hasHtml5Repo) push('    - {{ include "originalAppName" $ }}-html5-repo-runtime-bind')
  ...
  // === Tenant Operation Workload ===
  if (!isServiceOnly) {
    push(`  - name: tenant-job
    labels:
      sme.sap.com/app-type: {{ include "appName" $ }}
    consumedBTPServices:`)
  ...
  if (hasMultitenancy && hasXsuaa) push('    - {{ include "originalAppName" $ }}-saas-registry-bind')
  if (hasMultitenancy && hasIas) push('    - {{ include "originalAppName" $ }}-subscription-manager-bind')
  if (hasHtml5Repo) push('    - {{ include "originalAppName" $ }}-html5-repo-host-bind')
...
module.exports = { replacePlaceholders, mergeObj, isCAPOperatorChart, isConfigurableTemplateChart, isServiceOnlyChart, ask, injectTemplateFunction, transformValuesAndFillCapOpCroYaml, convertHypenNameToCamelcase, getConfigurableCapOpCroYaml }
