...
async function transformValuesAndFillCapOpCroYaml() {
  ...
  for (const [workloadKey, workloadDetails] of Object.entries(valuesYaml.workloads)) {
    ...
    const definitionKey = deploymentDefinition ? 'deploymentDefinition' : 'jobDefinition'
...
function getAllServiceInstanceKeyNames(serviceInstances, offeringName) {
  return Object.keys(serviceInstances).filter(
    key => serviceInstances[key].serviceOfferingName === offeringName
  )
}
