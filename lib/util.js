...
function isFlexibleTemplateChart(chartFolderPath) {
  try {
    const chartYaml = cds.parse.yaml(cds.utils.fs.readFileSync(chartFolderPath + "/Chart.yaml").toString())
    return chartYaml.annotations?.["app.kubernetes.io/part-of"] === 'cap-operator-flexible-templates' || false
  } catch (err) {
    return false
  }
}
...
module.exports = { replacePlaceholders, mergeObj, isCAPOperatorChart, isFlexibleTemplateChart, ask, injectTemplateFunction }
