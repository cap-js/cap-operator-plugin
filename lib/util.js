...
async function transformValuesAndFillCapOpCroYaml() {
  ...
  let workloadArray = []
  let newWorkloadObj = {}
  let tenantHostPattern = { name: 'TENANT_HOST_PATTERN', value: '^(.*).{{ template "domainPatterns" . }}' }
...
function removeSectionFromYaml(filepath, section) {
  yamlContent = cds.parse.yaml(cds.utils.fs.readFileSync(filepath).toString())
  ...
  cds.utils.fs.writeFileSync(filepath, result.join('\n'))
}

module.exports = { replacePlaceholders, mergeObj, isCAPOperatorChart, isConfigurableTemplateChart, ask, injectTemplateFunction, transformValuesAndFillCapOpCroYaml, convertHypenNameToCamelcase, removeSectionFromYaml }
