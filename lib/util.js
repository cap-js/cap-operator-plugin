...
async function ask(...args) {
  const questions = args.map(([message, initial, mandatory], index) => {
    const isMandatory = mandatory && !initial
    const indicator = isMandatory ? ' (required)' : ' (optional)'
    return {
      type: 'input',
      name: String(index),
      message: message + indicator,
      initial: initial || '',
      validate: isMandatory ? (value) => value?.trim() ? true : `This field is required. Please provide a value for: ${message}` : undefined,
      separator: ''
    }
  })
