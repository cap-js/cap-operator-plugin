...
module.exports = class MtaTransformer {
    ...
    async getWorkloads() {
        ...
        for (let module of modules) {
            ...
            if (module.path.includes("gen/mtx/sidecar")) {
                workload.jobDefinition = {
                    type: "TenantOperation",
                    image: null
                }
            } else if (module.type === 'com.sap.application.content') {
                workload.jobDefinition = {
                    type: "Content",
                    image: null
                }

                if (this.isHtml5ContentJobModule(module, workload)) {
                    workload.jobDefinition.env = [
                        { name: 'EXIT_PROCESS_AFTER_UPLOAD', value: "true" }
                    ]
                }
            ...
            if (module.properties) {
                ...
                if (workloadEnv.length > 0) {
                    let definitionKey = workload.deploymentDefinition ? 'deploymentDefinition' : 'jobDefinition'
                    if (workload[definitionKey].env) {
                        workload[definitionKey].env = workload[definitionKey].env.concat(workloadEnv)
                    } else {
                        workload[definitionKey].env = workloadEnv
                    }
                }
            }
            ...
            if (workload.deploymentDefinition?.type == "Router") {
                if (workload.deploymentDefinition.env) {
                    const index = workload.deploymentDefinition.env.findIndex(e => e.name === tenantHostPattern.name)
                    if (index == -1)
                        workload.deploymentDefinition.env.push(tenantHostPattern)
                    else
                        workload.deploymentDefinition.env[index].value = tenantHostPattern.value
                } else {
                    workload.deploymentDefinition.env = [tenantHostPattern]
                }
            }
    ...
    isHtml5ContentJobModule(module, workload) {
        let isHtml5ContentJobModule = false

        // check if any of the service bindings is html5-apps-repo service with app-host plan
        workload.consumedBTPServices.forEach((bindingName, index) => {
            this.serviceBindings.forEach((serviceBindingValue, serviceBindingKey) => {
                if (serviceBindingValue.name === bindingName) {
                    let serviceInstanceName = serviceBindingValue.serviceInstanceName
                    let serviceInstance = Array.from(this.serviceInstances.values()).find(si => si.name === serviceInstanceName)
                    if (serviceInstance && serviceInstance.serviceOfferingName === 'html5-apps-repo' && serviceInstance.servicePlanName === 'app-host') {
                        isHtml5ContentJobModule = true
                    }
                }
            })
        })

        return isHtml5ContentJobModule
    }
}
